...
import { setPage, show, hide } from '../../shared/ui.js';
import { setupApp } from './nav.js';
  ...
  try {
    ...
    if (error || !data) {
      ...
      if (authUser?.user) {
        ...
        const { data: newData } = await sb.from('usuarios').select('*').eq('id', currentUser.id).single();
        if (newData) { currentPerfil = newData.perfil; setupApp(newData); return; }
      }
      setPage('auth-screen');
      return;
    }
    currentPerfil = data.perfil;
    if (data.perfil === 'pendente') { setPage('pending-screen'); return; }
    setupApp(data);
  } catch(e) {
...
Object.assign(window, {
  switchAuthTab, doLogin, doRegister, selectRadio, toggleSenha,
  abrirRecuperarSenha, enviarRecuperacao, doLogout
});
